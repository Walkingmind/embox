...
#define BUFFER_CUR_CH(dev)		do { if ( BEG(dev) == MAX_BUFFER_SIZE ) { BEG(dev) = 0; } \
...
int pipe_read   ( device_t *dev , char *buf    , size_t n  ) {
	if ( PRIVATE(dev)->ioctl_base_flags | IOCTLP_NONBLOCKINGIO ) {
		/* non-blocking read (may read less chars than n) return how many bytes was read */
		int i;
...
int pipe_write  ( device_t *dev , char *buf    , size_t n  ) {
	if ( PRIVATE(dev)->ioctl_base_flags | IOCTLP_NONBLOCKINGIO ) {
		/* non-blocking write (may write only part) return how many bytes was written*/
