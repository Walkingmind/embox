...
void multipage_init(void) {
	...
	/* one byte for one page (no bit pack) */
	sizeoftree = 2 * rootblocksize;
	...
	maxblocksize = (rootblocksize == sizeofpool) ?
			rootblocksize : rootblocksize / 2;
...
void mpfree(void *ptr) {
	...
	if (get_bits(addr) == 4) { /* else if don't equal 0 algorithm has ERROR!!! */
		...
		for (; addr > 1;) {
			...
			parent = addr >> 1;
			/* unmark from left or right child for node */
			SET_BIT0(parent, addr & 1 ? 2 : 1);
...
#undef VAR
	...
	TRACE("\n\tTree:\n\t\t");
	for (ptr = HEAP_START_PTR;
			ptr < (HEAP_START_PTR + 2 * rootblocksize - 1); ++ptr) {
		TRACE("%ld ",*ptr);
	}
	TRACE("\n");
	TRACE("info end\n");
