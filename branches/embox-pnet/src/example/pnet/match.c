...
static int match_example(int argc, char **argv) {
	...
	net_node_t devs, match_node, lin_gate, info, skb, null;
	match_rule_t rule_icmp, rule_tcp;
	...
	rule_icmp = pnet_rule_alloc();
	pnet_rule_set_next_node(rule_icmp, info);
	pnet_rule_set_proto(rule_icmp, IPPROTO_ICMP);
	rule_icmp->priority = 1;
	pnet_add_new_rx_rule(rule_icmp, (net_node_matcher_t) match_node);

	rule_tcp = pnet_rule_alloc();
	pnet_rule_set_next_node(rule_tcp, info);
	pnet_rule_set_proto(rule_tcp, IPPROTO_TCP);
	rule_tcp->priority = 2;
	pnet_add_new_rx_rule(rule_tcp, (net_node_matcher_t) match_node);
