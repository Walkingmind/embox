 ...
 * by `zSig1Ptr'.
 */
static void normalizeFloat64Subnormal(bits32 aSig0, bits32 aSig1,
 ...
 */
float64 packFloat64(flag zSign, int16 zExp, bits32 zSig0, bits32 zSig1) {
	float64 z;
	z.low = zSig1;
 ...
 * than the ``true'' floating-point exponent.  The handling of underflow an
 * overflow follows the IEC/IEEE Standard for Binary Floating-Point Arithmetic
 */
static float64 roundAndPackFloat64(flag zSign, int16 zExp, bits32 zSig0,
						bits32 zSig1, bits32 zSig2) {
	int8 roundingMode;
	...
	if (0x7FD <= (bits16) zExp) {
		if ((0x7FD < zExp) || ((zExp == 0x7FD)
		    && eq64(0x001FFFFF, 0xFFFFFFFF, zSig0, zSig1) && increment)) {
			float_raise(float_flag_overflow | float_flag_inexact);
			if ((roundingMode == float_round_to_zero)
		...
		if (zExp < 0) {
			...
			shift64ExtraRightJamming(zSig0, zSig1, zSig2, - zExp, &zSig0, &zSig1, &zSig2);
			zExp = 0;
...
static float32 addFloat32Sigs(float32 a, float32 b, flag zSign) {
	...
	if (0 < expDiff) {
		if (aExp == 0xFF) {
			return aSig ? propagateFloat32NaN(a, b) : a;
		}
		if (bExp == 0) {
			--expDiff;
		} else {
		...
		if (bExp == 0xFF) {
			return bSig ? propagateFloat32NaN(a, b) : packFloat32(zSign, 0xFF, 0);
		}
		if (aExp == 0) {
			++expDiff;
		} else {
		...
		if (aExp == 0xFF) {
			return (aSig | bSig) ? propagateFloat32NaN(a, b) : a;
		}
...
bExpBigger:
	if (bExp == 0xFF) {
		return bSig ? propagateFloat32NaN(a, b) : packFloat32(zSign ^ 1, 0xFF, 0);
	}
	if (aExp == 0) {
		++expDiff;
	} else {
...
float32 float32_div(float32 a, float32 b) {
	...
	if (bExp == 0xFF) {
		return bSig ? propagateFloat32NaN(a, b) : packFloat32(zSign, 0, 0);
	}
	if (bExp == 0) {
		if (bSig == 0) {
			if ((aExp | aSig) == 0) {
...
int32 float64_to_int32(float64 a) {
	...
	if (roundingMode == float_round_nearest_even) {
		...
		aSigExtra = (aSigExtra != 0);
		if (aSign) {
			z = - (absZ + ((roundingMode == float_round_down) & aSigExtra));
		} else {
	...
	if (0x413 <= aExp) {
		...
		if (aExp <= 0x3FE) {
			...
			case float_round_nearest_even:
				...
				break;
			case float_round_down:
				return aSign ? packFloat64(1, 0x3FF, 0, 0)
					: packFloat64(0, 0, 0, 0);
			case float_round_up:
				return aSign ? packFloat64(1, 0, 0, 0)
...
flag float64_eq(float64 a, float64 b) {
	...
	return (a.low == b.low) && ((a.high == b.high) || ((a.low == 0)
...
flag float64_le(float64 a, float64 b) {
	...
	if (aSign != bSign) {
		return aSign
			|| ((((bits32)((a.high | b.high)<<1)) | a.low | b.low) == 0);
	}
	return aSign ? le64(b.high, b.low, a.high, a.low)
...
flag float64_lt(float64 a, float64 b) {
	...
	return aSign ? lt64(b.high, b.low, a.high, a.low)
...
flag float64_eq_signaling(float64 a, float64 b) {
	...
	return (a.low == b.low) && ((a.high == b.high)
		|| ((a.low == 0) && ((bits32) ((a.high | b.high)<<1) == 0)));
}
