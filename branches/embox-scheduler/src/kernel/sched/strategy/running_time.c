...
void sched_timing_init(struct thread *t) {
	...
	t->sched_priv.sched_time.running_time = 0;
	t->sched_priv.sched_time.last_sync = clock();
}
...
clock_t sched_timing_get(struct thread *t) {
	...
	if (thread_state_oncpu(t->state)) {
		running = clock() - t->sched_priv.sched_time.last_sync;
		running += t->sched_priv.sched_time.running_time;
	} else {
		running = t->sched_priv.sched_time.running_time;
	}
...
void sched_timing_stop(struct thread *t, clock_t cur_time) {
	...
	spent = cur_time - t->sched_priv.sched_time.last_sync;

	t->sched_priv.sched_time.running_time += spent;
	//TODO this is for qt
	t->task->per_cpu += spent;
}

void sched_timing_start(struct thread *t, clock_t cur_time) {
	t->sched_priv.sched_time.last_sync = cur_time;
}
