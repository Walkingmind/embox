...
typedef FLASH_FDATA * /*volatile */FLASH_FDATA_PTR;
#define FLASH_FDATA_SZ 4


#define D_ID_MANUF_CODE_OFFSET         0
#define D_ID_DEVID_OFFSET              FLASH_FDATA_SZ*1
#define D_ID_BLOCK_LOCK_STAT_OFFSET    FLASH_FDATA_SZ*2
 ...
 * @return FLASH_FDATA_PTR  - address returned in ptr form
 */
 ...
 * @return FLASH_STATUS
 */
FLASH_STATUS flash_lock_block (FLASH_DEV *flash_dev, uint16_t blocknum, uint8_t  returnSR );
 ...
 * @return FLASH_STATUS
 */
FLASH_STATUS flash_lock_block_down (FLASH_DEV *flash_dev, uint16_t blocknum, uint8_t  returnSR );
 ...
 * read mode.
 * Note:  If device is set to synchronous, use flash_set_configuration function
 *        to set burst sequence, burst length, clock edge, etc.) in the
 *        the Read Configuration Register.
 * @param enable - flag "1" to indicate the page mode is enable and
 ...
 * address with the single specified data value.
...
/**
 * This procedure is called to program the flash device at the specified
 * starting address continuously with the specified buffer data.
 * @param address  - the flash address to be programmed.
 ...
 * @return FLASH_STATUS
 */
FLASH_STATUS flash_program_flash_buffered (FLASH_DEV *flash_dev, uint32_t address, uint32_t *buffer, uint32_t numbytes, uint8_t returnSR );
 ...
 * @return FLASH_STATUS
 */
FLASH_STATUS flash_program_protection (FLASH_DEV *flash_dev, uint32_t location, FLASH_FDATA value, uint8_t returnSR );
 ...
 */
/*volatile */int flash_readf ( uint32_t address, uint32_t* value );
 ...
 * @return TMPL_Status
 */
FLASH_STATUS flash_read_flash (FLASH_DEV *flash_dev, uint32_t address, void *buffer, uint32_t numbytes, uint8_t  returnSR );
 ...
 * @return FLASH_STATUS
 */
FLASH_STATUS flash_unlock_block (FLASH_DEV *flash_dev, uint16_t blocknum, uint8_t  returnSR );
 ...
 * device should use the flash_program_flash or TMPL_ProgramFlashBuffered
 * command.
