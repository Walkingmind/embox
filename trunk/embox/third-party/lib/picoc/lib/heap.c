...
# else
static unsigned char HeapMemory[HEAP__SIZE];         /* all memory - stack and heap */
static void *HeapBottom = &HeapMemory[HEAP__SIZE];   /* the bottom of the (downward-growing) heap */
...
void *HeapAllocMem(int Size)
{
    ...
    { 
        ...
        assert((unsigned long)NewMem >= (unsigned long)&HeapMemory[0] && (unsigned char *)NewMem - &HeapMemory[0] < HEAP__SIZE);
        FreeListBucket[Bucket] = *(struct AllocNode **)NewMem;
        assert(FreeListBucket[Bucket] == NULL || ((unsigned long)FreeListBucket[Bucket] >= (unsigned long)&HeapMemory[0] && (unsigned char *)FreeListBucket[Bucket] - &HeapMemory[0] < HEAP__SIZE));
    ...
    { 
        ...
        if (*FreeNode != NULL)
        {
            assert((unsigned long)*FreeNode >= (unsigned long)&HeapMemory[0] && (unsigned char *)*FreeNode - &HeapMemory[0] < HEAP__SIZE);
            assert((*FreeNode)->Size < HEAP__SIZE && (*FreeNode)->Size > 0);
...
#endif
    assert((unsigned long)Mem >= (unsigned long)&HeapMemory[0] && (unsigned char *)Mem - &HeapMemory[0] < HEAP__SIZE);
    assert(MemNode->Size < HEAP__SIZE && MemNode->Size > 0);
    if (Mem == NULL)
        return;
...
#endif
        assert(FreeListBig == NULL || ((unsigned long)FreeListBig >= (unsigned long)&HeapMemory[0] && (unsigned char *)FreeListBig - &HeapMemory[0] < HEAP__SIZE));
