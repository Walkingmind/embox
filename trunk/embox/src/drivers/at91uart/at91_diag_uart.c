...
#include <AT91SAM7_US.h>
#include <AT91SAM7S256.h>

#define TTGR_DISABLE 0
#define RXRDY 1 /* Receive Ready */
#define TXRDY 2 /* Transive Ready */
...
int diag_has_symbol(void) {
	return (RXRDY & REG_LOAD(AT91C_US0_CSR));
}

void diag_putc(char ch) {
	while (!(TXRDY & REG_LOAD(AT91C_US0_CSR))) {  //while (!line_is_accepts_new_char)
	}
